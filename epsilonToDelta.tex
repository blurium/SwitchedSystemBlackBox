...
\begin{lemma}\label{lemma:delta2}Given a set $X \subseteq \sphere$, let $\conv(\sphere \setminus X) := \ball \cap \calP,$ where $\calP:=\bigcap\limits_{i \in I}\{x: c_i^Tx \leq k_i\}$. Then we have: $$\Delta(X) = \min_{i \in I} \frac{|k_i|}{\|c_i\|}.$$
\end{lemma}
...
\label{shrinkage1}&=&\min\left(\min_{i \in I} \frac{|k_i|}{\|c_i\|},1\right) \qquad \qquad\text{due to Lemma\,} 
{\ref{prop:distanceSet}}.
\end{eqnarray}
\end{proof}

\begin{lemma}\label{lemma:deltaMonotone}$\Delta(\calC_{c,k_1}) < \Delta(\calC_{c,k_2})$ when $k_1 < k_2$.
\end{lemma}

\begin{proof}
\begin{equation*}\Delta(\calC_{c,k}) = \conv(\sphere \setminus \calC_{c,k}) = \conv(\sphere \setminus \{x\in \sphere: c^Tx >k\}) = \{x \in \ball : c^Tx \leq k\}.\end{equation*} Due to Lemma \ref{lemma:delta2}, we have $\Delta(X) = \frac{|k|}{\|c\|}$.
Then since $\frac{|k_1|}{\|c\|} < \frac{|k_2|}{\|c\|}$ when $k_1 < k_2$, the result of the lemma follows.
\end{proof}
...
\begin{proof} As discussed previously, we can represent $\conv(\sphere \setminus  X)$ as
\begin{equation*}\conv(\sphere \setminus X) = \bigcap\limits_{i \in I} \{x \in \ball:c_i^Tx \leq k_i\}.
\end{equation*}
Note that when $X = \emptyset$, the statement of the lemma trivially holds since we can always find a $c$ and $k$ such that $\calC_{c, k} = \emptyset$, hence we assume $X \not= \emptyset$ for the rest of the proof. This implies $I \neq \emptyset$. Then due to Lemma \ref{lemma:delta2}, there exists $\ell \in I$ such that $\Delta(X)=\frac{|k_{\ell}|}{\|c_{\ell}\|}$. Now, consider the spherical cap $\calC_{c_\ell,k_\ell}$. Note that again due to Lemma \ref{lemma:delta2}, we have $\Delta(\calC_{c_\ell,k_\ell}) = \frac{|k_\ell|}{\|c_\ell\|} = \Delta(X)$.
...
\begin{theorem}Let $X_{\epsilon'} = \{X \subset \sphere: \sigma^{n-1}(X) = \epsilon'\}$. Then, for any $\epsilon' \in (0,1)$, the function $\Delta(X)$ attains its minimum over $X_{\epsilon'}$ when $X$ is a spherical cap.
\end{theorem}

\begin{proof}We prove this via contradiction. Assume that $X^* \in X_{\epsilon'}$, $X^*$ is not a spherical cap and $\argmin_{X \in X_{\epsilon'}}(\Delta(X))=X^*$. Due to Lemma \ref{lemma:constructSC} we can construct a spherical cap $\calC_{c,k}$ such that $\calC_{c,k} \subseteq X^*$ and $\calC_{c,k} = \Delta(X^*)$. Note that, we further have $\calC_{c,k} \subset X^*$, since $X^*$ is assumed not to be a spherical cap. This means that $\sigma^{n-1}(\calC_{c,k}) < \epsilon'$. 
...
\begin{corollary}
When $N \to \infty$, $\Delta(X) \to 1$.
\end{corollary}

\begin{proof}
To be proved: $\epsilon \to 0$.

Then by our assumption $\frac{\lambda_{\max}}{\lambda_{\min}}$ is bounded so $\epsilon'$ tends to $0$. By continuity and monotonicity of $I^{-1}$ in its first parameter, $\Delta$ tends to $1$.
