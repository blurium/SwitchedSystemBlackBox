...
for some $\sphere' \subset \sphere$ where $\bar{A_j} = L^{-1}A_jL$, where $L$ is defined as in \eqref{choleski} and $$\sigma(\sphere') \leq m\epsilon \sqrt{\frac{\lambda_{\max}(P)}{\det(P)^n}}.$$
\end{theorem}
