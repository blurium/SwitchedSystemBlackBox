...
%\end{theorem}
\begin{theorem}\label{thm:mainTheorem01}Consider a set of matrices $A \in \calM$, and an ellipsoid $E_P$, satisfying:
\begin{equation}(A_j x)^TP(A_j x) \leq {\gamma^*}^2x^TPx,\,\, \forall\, (x, j) \in Z \setminus V,\end{equation}
for some $V \subset Z$ where $\mu(V) \leq \epsilon$. Then, the following also holds: \begin{equation}(A_j x)^TP(A_j x) \leq {\gamma^*}^2x^TPx,\,\, \forall\, x \in E_P \setminus E', \forall\, j \in M,\end{equation}
...
\end{eqnarray*}
Note that we have $$\min_{j \in M} \mu_M(\{j\}) = \frac{1}{m}.$$ Then since $ \forall \ i$, $\mu_M(\calM_i) \geq \frac{1}{m}$, we have
\begin{equation}
\sigma^{n-1}(V_{\sphere}) \leq \frac{\mu(V)}{m_1}.
\end{equation}
\textcolor{blue}{Joris: Here, we need the discussion on $\sigma^{n-1}(V_{\sphere})$ being the same as $\sigma_P(E')$ for some $E':=???$ in $E_P$.}
\end{proof}

\begin{theorem}\label{thm:mainTheorem1}Consider a set of matrices $A \in \calM$, and an ellipsoid $E_P$ satisfying:
...
\begin{proof}
\textcolor{red}{Will look into this in the morning.}
...
%for some $P \succ 0$. 
%Note that the inequality \eqref{eqn:violation} in Theorem \ref{thm:campi} can be also written as:
...
%\begin{equation}\label{eqn:eps}\epsilon \leq I^{-1}(\beta, d+1, N-d),\end{equation} we have $\mathbb{P}^N\left\{ \mu(V(\omega_N)) \leq \epsilon \right\} \geq \beta$.
%
...
%\begin{equation*} (Ax)^TP(A x) \leq  {\gamma^*}^2 x^TPx, \quad \forall\, x \in E_P \setminus E',\, \forall A \in \calM, \end{equation*}
%for some $E'$ where $\sigma_P(E') \leq m \epsilon$ which can be rewritten as:
%\begin{equation*}\frac{\calM}{\gamma^*}(E_P \setminus E') \subset E_P.
%\end{equation*}
