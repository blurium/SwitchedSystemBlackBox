...
We now know how to compute an upper bound on the JSR when the ``almost invariant" ellipsoid is $\sphere$. Thanks to the homogeneity of the dynamics stated in Property \ref{property:homogeneity}, if this is not the case, we can simply perform a change of coordinates where this ellipsoid is mapped to $\sphere$ and compute the JSR in the new coordinates system instead. To do this, in the next theorem, we bound the measure of violating constraints on $\sphere$ after the change of coordinates, in terms of the measure of the violated constraints on $\sphere \times M$ in the original coordinates.
