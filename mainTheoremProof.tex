...
Even though the solution of the optimization problem in \eqref{eqn:campiOpt0} provides a CQLF, solving this problem as stated is not possible since it involves infinitely many constraints. Nevertheless, we are going to show that the solution of the Optimization problem \eqref{eq:lowerbound} allows us to not only compute a lower bound, but also a (probabilistic) upper bound.
...
\begin{theorem}\label{thm:mainTheorem01}Let $\gamma \in \R_{> 0}$. Consider a set of matrices $A \in \calM$, and a matrix $P \succ 0$ satisfying:
