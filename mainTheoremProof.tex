% !TEX root = main.tex
\begin{theorem}[Main Theorem] \label{thm:mainTheorem} Given $N \geq n+1$ samples of the switched linear system \eqref{blah} and $\beta \in [0,1)$, we can compute $\delta(N) < \infty$ such that with probability at least $\beta$, $\rho \leq \frac{\gamma^*}{\delta}$. Moreover, $\lim_{N \to \infty} \delta(N) = 1$.
\end{theorem}
%\begin{theorem}
