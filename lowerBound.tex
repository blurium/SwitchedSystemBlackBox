...
\begin{theorem}
For a given uniform sampling: $$\omega_N := \{(x_1, j_1), (x_2, j_2), \ldots, (x_N, j_N)\} \subset \R^n \times M,$$ let $S_{\omega_N}=\{(x_1,y_1),...,(x_N,y_N)\}$ be the corresponding observations available, which satisfies $$y_i= A_{j_i}x_i\,\, \forall (x_i, y_i) \in S_{\omega_N}.$$Also let $\gamma^*(\omega_N)$ be the optimal solution of the following optimization problem:
