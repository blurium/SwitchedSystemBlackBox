...
\end{equation}
where, $x_k \in X$ is the state and $k \in \N$ is the time index. In this paper, we focus on switched systems, but we believe the presented can be extended to a more general class of dynamical systems as well.
...
\end{bmatrix}x,
\]
admits a Lyapunov function candidate on the unit circle except the two red areas shown in Figure \ref{fig:levelset}.
