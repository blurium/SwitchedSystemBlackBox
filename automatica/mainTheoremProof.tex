...
P = L^TL,
\end{equation} 
where $L$ is an upper triangular matrix. Note that, $L$ maps the elements of \ayca{$E_P$ to $\sphere$.}
...
Finally, thanks to the \ayca{This doesn't immediately follow from this property, that property is immediately on the JSR, no? It follows from homegenity of the dynamics.} Property~\ref{rem:scaling}, we even have:
