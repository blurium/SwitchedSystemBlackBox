...
Since we have seen in the previous section a technique to solve the spherical case, we perform now the change of coordinates defined as in \eqref{cholesky} by $L^{-1} \in \mathcal{L}(\mathbb{R}^n)$ which maps the ellipsoid $E_P$ to the sphere $\sphere$. By defining $\bar{A}_{j_i}=  L A_{j_i} L^{-1}$, assumption \eqref{eq:assumptionThm01} becomes:
\begin{equation}\label{eq:assumption3}
\begin{aligned}
& (\bar{A}_{j_l} \bar{A}_{j_{l-1}} \dots \bar{A}_{j_1} x)^T (\bar{A}_{j_l} \bar{A}_{j_{l-1}} \dots \bar{A}_{j_1} x) \leq ({\gamma^*})^{2l} x^T x, \\
