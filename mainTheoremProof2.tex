...
\begin{equation*} (A_jx)^TP(A_jx) \leq  {\gamma^*}^2 x^TPx, \quad \forall x \in E_P \setminus E', \forall\, j \in M,\end{equation*}
for some $E'$ where $\sigma_P(E') \leq m \epsilon$.  Then, applying Theorem \ref{thm:mainTheorem1}, we can compute $$\delta(\beta, \omega_N)= \alpha(\epsilon) = \alpha(1- I^{-1}(\beta;d+1,N-d)),$$ such that with probability at least $\beta$ we have:
\begin{equation*}
\frac{A_j}{{\gamma^*}^2} E_P \subset \delta(\beta, \omega_N)E_P, \forall\, j \in M,
\end{equation*}
where $\lim_{N \to \infty}\delta(\beta, \omega_N) = 1.$
This means that $P$ defines a CQLF for the switched system \eqref{eq:switchedSystem} since we have $\frac{A_j}{\gamma^*}^TP\frac{A_j}{\gamma^*} \preceq P,\, \forall\, j$, due to Property \ref{rem:scaling}, the followings holds with probability at least $\beta$:
$$\rho \leq \frac{\gamma^*(\omega_N) (1 + \eta)}{\delta(\beta, \omega_N)},$$
