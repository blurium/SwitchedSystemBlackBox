...
\begin{abstract}
We explore the general problem of deciding stability of a "black-box system", that is, a system whose model equations are not known.  The only information at our possession is a set of observations, that is, couples of vectors of the type $(x(k), x(k+1)).$  We adopt a probabilistic approach, and focus on switched systems, which are a widely used model for many complex systems, and are well known to be hard to analyze, even in a non-'black-box setting'.  
