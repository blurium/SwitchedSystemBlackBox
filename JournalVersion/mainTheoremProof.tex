...
In Theorem~\ref{thm:mainTheorem01} below, we transform this question into a geometric problem, which we now describe.  First, we apply a change of coordinates bringing $E_P$ to $\sphere$. Since $P \in \calS_{++}^n$, it can be written in its Choleski form 
...
Raphael:
"In Figure X we illustrate this problem with $X$ being the union of two \emph{spherical caps.}}
...
By exploiting Property \ref{property:convpres} above, we first show in Lemma \ref{lemma:epsilon1} how one can compute an upper bound on the JSR when the ``almost invariant" set is the unit sphere $\sphere$.
