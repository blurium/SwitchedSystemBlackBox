...
%
%
%here we could define properly the problem we have, namely 
%'The above theorem allows us to conclude, from a finite number of observations, that the required property is actually true for the complete S, except a small set, whose measure we can bound by a number $\epsilon.$  Thus, we are left with the following question: `What can we conclude on the JSR if we can assume that the Lyapunov property is satisfied by all points, except a set of measure $\epsilon?$'
%
%In Theorem [currently Proposition 4.3] below, we transform this question into a geometric problem, which we now describe.
%We are given a subset X of the sphere, and we want to include the largest ball in $\convhull (S \setminus X). In Figure X we illustrate this problem with $X$ being the union of two \emph{spherical caps.}
%
%Then come two definitions:
% I would define here a spherical cap, motivating it by saying that it is our worst case set for a given measure. 
%Then, we define the function alpha in terms of the volume of a spherical cap of surface epsilon.  I would say that it can be computed numerically thanks to simple expressions of well known canonical functions (see appendix.)
%
%Then I would end by saying 
%'It turns out that spherical caps are actually the worst case for our problem above, more precisely: 
%cite lemma A.3/proposition A.4, currently in appendix, ( I actually don't see the difference), saying that the proof is in appendix
%
%I'd leave in appendix the computations in lemma A.5, (and maybe also the formula?)
%
