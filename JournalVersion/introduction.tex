...
We start with the following question to serve as a stepping stone: For some $l \in \N_{>0}$, given $N$ traces of length $l$, $(x_0^1,x_1^1,\ldots, x_l^1)$, $(x_0^2,x_1^2,\ldtos, x_l^2)$, $\ldots$, $(x_0^N,x_1^N,\ldtos,x_l^N)$ belonging to the behavior of the system \eqref{eq:dynamicalsystemGeneral}, (i.e., $x_{k+1}^i = f(k, x_k^i)$ for any $k \in \{0,\ldots,l-1\}$ and any $i \in \{1,\ldots,N\}$), what can we say about the stability of the system \eqref{eq:dynamicalsystemGeneral}? For the rest of the paper, we use the term \emph{black-box} to refer to models where we do not have access to the model, i.e., to $f$, yet we can indirectly learn information about $f$ by observing traces of length $l$ (becoming, in the particular case where $l=1$, couples of points $(x_k, y_k)$ as defined in \eqref{eq:dynamicalsystemGeneral}).
