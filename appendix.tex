...
& \leq & \lambda_{\max}(\gamma^*(Z), \omega_{d+1}),
\end{eqnarray*}
where the first inequality follows from the fact that \mbox{$\gamma^*(Z) \leq \gamma^*(\omega_N)$.} The second inequality follows from the inclusion $\omega_{d+1} \subset\omega_N$, and the fact that as we add less constraints the value of the objective function of \eqref{eqn:appendix} increases. Let: $$\bar{\lambda}_{\max} = c_{\max}(\gamma^{*}(Z), \omega_{d+1}).$$
