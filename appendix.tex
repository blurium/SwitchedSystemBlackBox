...
We now show that for a fixed $\beta \in (0,1]$ $\lim_{N \to \infty} \Phi(\beta, N) = 0.$ Note that, this is equivalent to showing that for a fixed $\beta \in (0, 1]$, $\lim_{N \to \infty}\epsilon(\beta, N) = 0$, where $\epsilon(\beta, N)$ is intrinsically defined by the following equation:
...
$\lim_{N \to \infty} m \kappa(P(\omega_N))\Phi(\beta, \omega_N) = 0,$ which implies by the continuity of the function $I^{-1}$ this implies: $\lim_{N \to \infty} \alpha(m \kappa(P(\omega_N))\Phi(\beta, \omega_N)) = 1.$
\end{subsection}
