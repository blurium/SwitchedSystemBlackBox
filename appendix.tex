% !TEX root = main.tex
The proof of the theorem is two-part. We first prove the statement of the theorem when $P=I$, i.e., when $E_P = \sphere$. (Lemma \ref{lemma:epsilon1}) Then, to apply this on an arbitrary ellipsoid defined by $P$, via a linear transformation mapping $L \in\calL(\R^n)$, we transform the problem into a new coordinates system, such that the ellipsoid $E_P$ maps to a sphere and compute an upper bound on the measure of the image of the set $E'$ in this new coordinate system. (Lemma \ref{lemma:epsilon2}) We use Remark ?? to tie these two results.

Before proving the first main lemma of the Appendix, we introduce the necessary preliminary definitions and related background.
...
\begin{theorem}Let $X$ be a spherical cap such that $\sigma^{n-1}(X) \leq \epsilon$ for some $\epsilon \in (0, 1)$, then the following holds:
$\ball_(\alpha(\epsilon))^2 \subset \conv(\sphere \setminus X)$,
where  $\alpha := I^{-1}\left(\frac{\epsilon'\Gamma(\frac{n}{2})}{\pi^{n/2}}, \frac{n-1}{2}, \frac{1}{2}\right)$, and $$\Gamma(x)=\int_{0}^{\infty} t^{x-1} e^{-t} \text{d}t.$$ Here $I^{-1}$ is the inverse incomplete beta function, i.e.,  $I^{-1}(y; a,b)= x$ where $I(x;a,b)=y$.
\end{theorem}
...
\begin{lemma}\label{lemma:epsilon1}Let $\epsilon \in (0, 1)$. Consider the set of matrices and $A \in \calM$, and a sphere $\sphere$ satisfying:
...
\begin{equation*}
A_j \sphere \subset \alpha(\epsilon) \sphere,\, \forall\,j \in M.
\end{equation*}
\end{lemma}
...
By Remark ?? this implies:
$$A_j (\convhull (\sphere \setminus \sphere')) \subset \ball.$$
Then by Lemma \ref{thm:mainSphericalCap}, we have
$$A_j \ball_{\delta()} \subset \ball,$$
This means that $\frac{A_j}{\sqrt{\alpha(\epsilon)}}^T\frac{A_j}{\sqrt{\alpha(\epsilon)}} \preceq I$, which 
implies that the statement of the theorem holds for $\sqrt(\alpha())$
\end{proof}
