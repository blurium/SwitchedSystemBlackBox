...
\begin{proposition}\label{thm:mainSphericalCap}Let $\calX_{\epsilon} = \{X \subset \sphere: \sigma^{n-1}(X) = \epsilon\}$. Then, for any $\epsilon \in (0,1)$, the function $\Delta(X)$ attains its minimum over $\calX_{\epsilon}$ for some $X$ which is a spherical cap.
\end{proposition}
...
\end{equation}
where we recall that $\ball$ (respectively $\sphere$) denote the unit ball (respectively unit sphere), $\calX_{\epsilon} = \{X \subset \sphere: \sigma^{n-1}(X) = \epsilon\}$ and $\Delta(X)$ is defined as in  \eqref{shrinkage}.
