...
We now show that for a fixed $\beta \in (0,1]$ $\lim_{N \to \infty} \Phi(\beta, N) = 0.$ Note that, this is equivalent to showing that for a fixed $\beta \in (0, 1]$, $\lim_{N \to \infty}\epsilon(\beta, N) = 0$, where $\epsilon(\beta, N)$ is also intrinsically defined by the following equation:
$1-\beta = \sum_{j=0}^d {{N}\choose{j}} \epsilon^j (1-\epsilon)^{N-j}.$
We can then upper bound the $1-\beta$ as in:
...
$\lim_{N \to \infty} m \kappa(P(\omega_N))\epsilon(\beta, \omega_N) = 0,$ which implies by the continuity of the function $I^{-1}$ this implies: $\lim_{N \to \infty} \alpha(m \kappa(P(\omega_N))\epsilon(\beta, \omega_N)) = 1.$
\end{subsection}
