...
\begin{lemma}\label{lemma:eps}Let $\epsilon \in (0, 1)$. Let $\alpha(\epsilon)$ be defined as in:
\begin{equation}\label{eqn:ballinc}\alpha(\epsilon) = \sup_{x \in \calX_\epsilon} \{r: r\ball \subset \convhull(\sphere \setminus X)\},
\end{equation}
where $\calX_{\epsilon} = \{X \subset \sphere: \sigma^{n-1}(X) \leq \epsilon\}$ and $\Delta(X)$ is defined as in  \eqref{shrinkage}. Then, we can compute $\alpha(\epsilon)$ by the formula:
