...
Recall that $\gamma^*(\omega_N)$ is the optimal solution to the problem \eqref{eq:lowerbound}. Note that, since $\omega_N$ is sequential, for any $t \in \N_{>0}$ such that \mbox{$d < t \leq N$} we have \mbox{$\gamma^*(\omega_t) \geq \gamma^*(\omega_N) \leq \gamma*_{Z}$.} Also note that,
\begin{equation*}\lambda_{\max}(\gamma^*(\omega_N), \omega_N) \leq \lambda_{\max}(\gamma^*(\omega_t), \omega_N) \lambda_{\max}(\gamma^*(\omega_Z)).
\end{equation*}
